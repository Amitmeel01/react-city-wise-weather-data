...
//           <div className="icons">
//             <h3 className='count'>{`${weather.name}, ${weather.country}`}</h3>
//             <img src={weather.iconUrl} alt="" />
//             <h3>{weather.description}</h3>
//           </div>
//           {/* Temperature */}
...
const WeatherPage = ({ setBg }) => {
  const { cityName } = useParams();
  const [weather, setWeather] = useState(null);
  const [temperatureUnit, setTemperatureUnit] = useState('metric'); // Default to Celsius (metric)
  const [inputValue, setInputValue] = useState(cityName);

  useEffect(() => {
    const fetchWeatherData = async () => {
      const data = await getweatherdata(cityName, temperatureUnit);
      setWeather(data);
      const bgtemp = temperatureUnit === "metric" ? 21 : 70;
      setBg(data.temp <= bgtemp ? cold : hot);
    };

    fetchWeatherData();
  }, [cityName, temperatureUnit, setBg]);
    ...
    <div className="container">
      ...
      {weather && (
        <div className="section temp">
          ...
          {/* Temperature */}
          <div className="temperature">
            <h1>{`${weather.temp.toFixed()}°${temperatureUnit === "metric" ? "C" : "F"}`}</h1>
          </div>
        </div>
      )}
      {/* Additional weather details */}
      <Description weather={weather} units={temperatureUnit} />
    </div>
  );
};
